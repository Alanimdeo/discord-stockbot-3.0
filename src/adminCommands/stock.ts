      ...
      if (isNaN(Number(command[0]))) {
        switch (command[0].toLowerCase()) {
          case "fetch":
            if (isNaN(Number(command[1]))) {
              ...
              const code = Object.values(bot.corpList).find(
                (code) => code == command[1]
              );
              if (code) {
                await message.reply(
                  Object.keys(bot.corpList)[
                    Object.values(bot.corpList).indexOf(code)
                  ]
                );
              } else {
                await message.reply("Not Found");
        ...
        switch (command[1].toLowerCase()) {
          ...
          case "add":
            ...
            break;
          case "reduce":
            await userdata.stock.reduceStock(
              command[2],
              Number(command[3]),
              Number(command[4])
            );
