        ...
        .addSubcommand((option) =>
            option
                ...
                .addNumberOption((option) =>
                    option.setName("금액").setDescription("보낼 급액을 입력하세요.").setRequired(true)
                )
        ),
...
async function 송금(interaction) {
    ...
    database.query(
        `SELECT id, money FROM users WHERE id = ${interaction.options.getUser("대상").id}`,
                ...
                await interaction.reply({
                    embeds: [
                        new MessageEmbed()
                            .setColor("#ff0000")
                            .setTitle(":warning: 오류")
                            .setDescription("받을 상대가 가입되어 있지 않습니다."),
                    ],
                });
            } else {
                        ...
                        if (interaction.options.getNumber("금액") > result_sender.money) {
                            ...
                            database.query(
                                `UPDATE users SET money = ${
                                    result_target.money + interaction.options.getNumber("금액")
