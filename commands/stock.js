    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((option) =>
            option
                ...
                .addStringOption((option) => option.setName("회사명").setDescription("회사명을 입력하세요. (종목코드도 가능)").setRequired(true))
        )
        .addSubcommand((option) => option.setName("내주식").setDescription("보유한 주식을 확인합니다."))
        .addSubcommand((option) =>
            option
                .setName("구매")
                .setDescription("주식을 구매합니다.")
                .addStringOption((option) => option.setName("회사명").setDescription("회사명을 입력하세요. (종목코드도 가능)").setRequired(true))
                .addNumberOption((option) => option.setName("수량").setDescription("수량을 입력하세요.").setRequired(true))
        )
        .addSubcommand((option) =>
            option
                .setName("판매")
    ...
    getStockData(interaction, interaction.options.getString("회사명"), async (code, stockData, company) => {
        await interaction.reply({
            embeds: [
                new MessageEmbed()
                    .setColor("#0090ff")
                    .setTitle(`:chart_with_upwards_trend: ${company}(${code})의 현재 주가`)
                    .setDescription(`\`${stockData.nv.toLocaleString("ko-KR")}원\``)
                    .setImage(`https://ssl.pstatic.net/imgfinance/chart/item/area/day/${code}.png?sidcode=${new Date().getTime()}`),
            ],
        });
    });
}
                ...
                await interaction.reply({
                    embeds: [
                        new MessageEmbed()
                            .setColor("#ff0000")
                            .setTitle(":warning: 오류")
                            .setDescription("없는 기업입니다. 회사명 또는 종목코드를 알맞게 입력하였는지 확인하세요."),
                    ],
                });
            } else {
...
async function 내주식(interaction) {
    ...
    database.query(`SELECT stock FROM users WHERE id = ${userID}`, async (err, result) => {
        ...
        if (result.length > 0) {
                ...
                for (var i in userStock) {
                    getStockData(interaction, i, async (code, stockData) => {
                        ...
                        if (avgPrice < nowValue) {
                            willSendMessage += `\`\`\`diff\n- ${name}(${code}): ${query.amount.toLocaleString(
                                "ko-KR"
                            )}주 (평균 구매가 ${avgPrice.toLocaleString("ko-KR")}원, 현재 ${nowValue.toLocaleString("ko-KR")}원)[+${(
                                query.amount * nowValue -
                                query.buyPrice
                            ).toLocaleString("ko-KR")}원, +${((((query.amount * nowValue) / query.buyPrice) * 100).toFixed(2) - 100).toLocaleString(
                                "ko-KR"
                            )}%]`;
                            ...
                            willSendMessage += `\`\`\`yaml\n= ${name}(${code}): ${query.amount.toLocaleString(
                                ...
                                query.amount * nowValue -
                                query.buyPrice
                            ).toLocaleString("ko-KR")}원, -${(((query.buyPrice / (query.amount * nowValue)) * 100).toFixed(2) - 100).toLocaleString(
                                "ko-KR"
                            )}%]`;
                        } else {
                            willSendMessage += `\`\`\`\n* ${name}(${code}): ${query.amount.toLocaleString("ko-KR")}주 (평균 구매가 ${avgPrice.toLocaleString(
                                "ko-KR"
                            )}원, 현재 ${nowValue.toLocaleString("ko-KR")}원)[=]`;
                        }
            ...
            await interaction.reply({
                embeds: [new MessageEmbed().setColor("#ff0000").setTitle(":warning: 오류").setDescription("없는 유저입니다.")],
            });
        }
    });
}
    ...
    getStockData(interaction, interaction.options.getString("회사명"), async (code, stockData, company) => {
        database.query(`SELECT money, stock FROM users WHERE id = ${interaction.member.id}`, async (err, result) => {
            ...
            if (price * amount > money) {
                await interaction.reply({
                    embeds: [new MessageEmbed().setColor("#ff0000").setTitle(":warning: 오류").setDescription("돈이 부족합니다.")],
                });
            } else if (amount < 1) {
                await interaction.reply({
                    embeds: [new MessageEmbed().setColor("#ff0000").setTitle(":warning: 오류").setDescription("1개 미만은 구매할 수 없습니다.")],
                });
            } else {
                if (userStock[code]) {
                        ...
                        await interaction.reply({
                            embeds: [
                                new MessageEmbed()
                                    ...
                                    .setDescription(
                                        `${company}(${code}) 주식을 구매했습니다.\n구매 금액: \`${price.toLocaleString("ko-KR")} × ${amount.toLocaleString(
                                            "ko-KR"
                                        )} = ${(price * amount).toLocaleString("ko-KR")}원\`\n보유 중인 주식: \`${userStock[code].amount.toLocaleString(
                                            "ko-KR"
    ...
    getStockData(interaction, interaction.options.getString("회사명"), async (code, stockData, company) => {
        database.query(`SELECT money, stock FROM users WHERE id = ${interaction.member.id}`, async (err, result) => {
            ...
            if (userStock[code] === undefined) {
                await interaction.reply({
                    embeds: [
                        new MessageEmbed().setColor("#ff0000").setTitle(":warning: 오류").setDescription(`${company}(${code})의 주식을 가지고 있지 않습니다.`),
                    ],
                });
            } else if (amount > userStock[code].amount) {
                await interaction.reply({
                    embeds: [new MessageEmbed().setColor("#ff0000").setTitle(":warning: 오류").setDescription(`가진 주식보다 많이 판매할 수 없습니다.`)],
                });
            } else if (amount < 1) {
                await interaction.reply({
                    embeds: [new MessageEmbed().setColor("#ff0000").setTitle(":warning: 오류").setDescription("1개 미만은 판매할 수 없습니다.")],
                });
            } else {
                userStock[code].amount -= amount;
                userStock[code].buyPrice -= price * amount;
