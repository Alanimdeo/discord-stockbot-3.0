        ...
        .addSubcommand((option) =>
            option
                ...
                .addStringOption((option) =>
                    option.setName("회사명").setDescription("회사명을 입력하세요. (종목코드도 가능)").setRequired(true)
                )
        )
        ...
        await interaction.reply({
            embeds: [
                new MessageEmbed()
                    ...
                    .setImage(
                        `https://ssl.pstatic.net/imgfinance/chart/item/area/day/${code}.png?sidcode=${new Date().getTime()}`
                    ),
            ],
        });
    });
                    ...
                    getStockData(interaction, i, async (code, stockData) => {
                        ...
                        if (avgPrice < nowValue) {
                            willSendMessage += `\`\`\`diff\n- ${name}(${code}): ${query.amount.toLocaleString(
                                "ko-KR"
                            )}주 (평균 구매가 ${avgPrice.toLocaleString("ko-KR")}원, 현재 ${nowValue.toLocaleString(
                                "ko-KR"
                            )}원)[+${(query.amount * nowValue - query.buyPrice).toLocaleString("ko-KR")}원, +${(
                                (((query.amount * nowValue) / query.buyPrice) * 100).toFixed(2) - 100
                            ...
                            willSendMessage += `\`\`\`yaml\n= ${name}(${code}): ${query.amount.toLocaleString(
                                ...
                                ((query.buyPrice / (query.amount * nowValue)) * 100).toFixed(2) - 100
                            ).toLocaleString("ko-KR")}%]`;
                        } else {
                            willSendMessage += `\`\`\`\n* ${name}(${code}): ${query.amount.toLocaleString(
                                "ko-KR"
                        ...
                        if (processed === Object.keys(userStock).length) {
                            return await interaction.editReply(willSendMessage);
                        }
                    });
                }
            });
    ...
    getStockData(interaction, interaction.options.getString("회사명"), async (code, stockData, company) => {
        database.query(`SELECT money, stock FROM users WHERE id = ${interaction.member.id}`, async (err, result) => {
            ...
            if (price * amount > money) {
                ...
                database.query(
                    `UPDATE users SET money = ${money - price * amount}, stock = '${JSON.stringify(
                        userStock
                        ...
                        await interaction.reply({
                            embeds: [
                                new MessageEmbed()
                                    ...
                                    .setDescription(
                                        `${company}(${code}) 주식을 구매했습니다.\n구매 금액: \`${price.toLocaleString(
                                            "ko-KR"
                                        )} × ${amount.toLocaleString("ko-KR")} = ${(price * amount).toLocaleString(
                                            "ko-KR"
            ...
            if (userStock[code] === undefined) {
                await interaction.reply({
                    embeds: [
                        new MessageEmbed()
                            .setColor("#ff0000")
                            .setTitle(":warning: 오류")
                            .setDescription(`${company}(${code})의 주식을 가지고 있지 않습니다.`),
                    ],
                });
                ...
                await interaction.reply({
                    embeds: [
                        new MessageEmbed()
                            .setColor("#ff0000")
                            .setTitle(":warning: 오류")
                            .setDescription(`가진 주식보다 많이 판매할 수 없습니다.`),
                    ],
                });
            } else if (amount < 1) {
                await interaction.reply({
                ...
                database.query(
                    `UPDATE users SET money = ${
                        money + price * amount - Math.round(price * amount * 0.003)
                    }, stock = '${JSON.stringify(userStock)}' WHERE id = ${interaction.member.id}`,
                    async (err) => {
                        if (err) console.error(err);
                        await interaction.reply({
                            embeds: [
