...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((option) =>
            option.setName("내주식").setDescription("보유한 주식을 확인합니다.")
        )
        .addSubcommand((option) =>
            option
                .setName("구매")
...
async function 확인(interaction) {
    getStockData(
        interaction,
        interaction.options.getString("회사명"),
        async (code, stockData, company) => {
            await interaction.reply({
                embeds: [
                    new MessageEmbed()
                        .setColor("#0090ff")
                        .setTitle(
                            `:chart_with_upwards_trend: ${company}(${code})의 현재 주가`
                        )
                        .setDescription(
                            `\`${stockData.nv.toLocaleString("ko-KR")}원\``
                        )
                        .setImage(
...
function getStockData(interaction, requestedCode, callback) {
    ...
    http.get(
        "http://polling.finance.naver.com/api/realtime?query=SERVICE_ITEM:" +
            code,
        (resp) => {
            var data = "";
            resp.on("data", (chunk) => {
                    ...
                    await interaction.reply({
                        embeds: [
                            new MessageEmbed()
                                ...
                                .setTitle(":warning: 오류")
                                .setDescription(
                                    "없는 기업입니다. 회사명 또는 종목코드를 알맞게 입력하였는지 확인하세요."
                                ),
                        ],
                    });
                } else {
...
async function 내주식(interaction) {
    ...
    const corpList = require("../corpList.json");
    database.query(
        `SELECT stock FROM users WHERE id = ${userID}`,
        async (err, result) => {
                    ...
                    .then(async (userInfo) => {
                        ...
                        for (var i in userStock) {
                            getStockData(
                                interaction,
                                i,
                                async (code, stockData) => {
                                    let nowValue = stockData.nv;
                                    let name =
                                        Object.keys(corpList)[
                                            Object.values(corpList).indexOf(
                                                code
                                            )
                                        ];
                                    let query = userStock[code];
                                    let avgPrice = Math.floor(
                                        query.buyPrice / query.amount
                                    );
                                    if (avgPrice < nowValue) {
                                        willSendMessage += `\`\`\`diff\n- ${name}(${code}): ${query.amount.toLocaleString(
                                            "ko-KR"
                                        )}주 (평균 구매가 ${avgPrice.toLocaleString(
                                            "ko-KR"
                                        )}원, 현재 ${nowValue.toLocaleString(
                                            "ko-KR"
                                        )}원)[+${(
                                    ...
                                    if (stockData.tyn === "Y")
                                        willSendMessage += `[거래정지]`;
                                    willSendMessage += "```";
                                    processed++;
                                    if (
                                        processed ===
                                        Object.keys(userStock).length
                                    ) {
                                        await interaction.reply(
                                            willSendMessage
                                        );
            ...
            database.query(
                ...
                async (err, result) => {
                    ...
                    if (userStock[code] === undefined) {
                        await interaction.reply({
                            embeds: [
                                new MessageEmbed()
                                    ...
                                    .setTitle(":warning: 오류")
                                    .setDescription(
                                        `${company}(${code})의 주식을 가지고 있지 않습니다.`
                                    ),
                            ],
                        });
                        ...
                        if (userStock[code].amount === 0)
                            delete userStock[code];
                        database.query(
                            `UPDATE users SET money = ${
                                money + price * amount
                            }, stock = '${JSON.stringify(
