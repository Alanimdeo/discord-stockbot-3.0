...
module.exports = {
    data: new SlashCommandBuilder().setName("가입").setDescription("주식 서버에 가입합니다. 최초 1회만 필요합니다."),
    async execute(interaction) {
        database.query(`SELECT * FROM users WHERE id = ?`, [interaction.member.id], async (err, result) => {
            ...
            if (result.length > 0) {
                ...
                database.query(
                    `INSERT INTO users (id, money, stock, lottery, lastClaim) VALUES (?, 1000000, JSON_OBJECT(), '[]', NOW())`,
