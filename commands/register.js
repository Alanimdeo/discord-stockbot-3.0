...
module.exports = {
    ...
    async execute(interaction) {
        await database.query(`SELECT * FROM users WHERE id = ${interaction.member.id}`, async (err, result) => {
			if (err) console.error(err);
			console.log(result);
			if (result.length > 0) {
				await interaction.reply({ embeds: [new MessageEmbed().setColor('#ff0000').setTitle(':warning: 오류').setDescription('이미 가입되어 있습니다.')] });
			} else {
                await database.query(`INSERT INTO users (id, money, stock, lottery, lastClaim) VALUES (${interaction.member.id}, 1000000, JSON_OBJECT(), JSON_OBJECT(), NOW())`, async err => {
                    if (err) console.error(err);
