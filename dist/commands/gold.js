...
async function 시세(interaction, bot) {
    ...
    let sellPriceDiff = price.sell.diff;
    if (sellPriceDiff === "0") {
        sellPriceDiff = `=`;
    }
    ...
    await interaction.editReply((0, types_1.Embed)({
        ...
        icon: "coin",
        title: "금 시세",
        description: `살 때: \`${price.buy.price.toLocaleString("ko-KR")}원[${buyPriceDiff}]\`\n팔 때: \`${price.sell.price.toLocaleString("ko-KR")}원[${sellPriceDiff}]\``,
    ...
    try {
        ...
        if (userdata.gold.amount > 0) {
            ...
            if (gold.sell.price !== userdata.gold.buyPrice) {
                ...
                diff = `${prefix}${(gold.sell.price - userdata.gold.buyPrice).toLocaleString("ko-KR")}원 (${prefix}${Math.round((userdata.gold.buyPrice / userdata.gold.amount / gold.sell.price - 1) * 10000 + Number.EPSILON) /
                    100}%)`;
            }
        }
        await interaction.editReply((0, types_1.Embed)({
            ...
            description: `수량: \`${userdata.gold.amount.toLocaleString("ko-KR")}원\`\n평균 구매가: : \`${(Math.round((userdata.gold.buyPrice / userdata.gold.amount + Number.EPSILON) * 100) / 100).toLocaleString("ko-KR")}원\`\n수익: \`${diff}\``,
        }));
    }
    catch (err) {
...
async function 구매(interaction, bot) {
    try {
        ...
        await interaction.editReply((0, types_1.Embed)({
            ...
            title: "구매 완료",
            description: `금 ${amount}개를 구매했습니다.\n구매 금액: \`${gold.buy.price.toLocaleString("ko-KR")} × ${amount.toLocaleString("ko-KR")} = ${(gold.buy.price * amount).toLocaleString("ko-KR")}원\`\n보유 중인 금: \`${userdata.gold.amount.toLocaleString("ko-KR")}개\`\n남은 돈: \`${userdata.money.amount.toLocaleString("ko-KR")}원\``,
        }));
    }
    catch (err) {
        await interaction.editReply((0, types_1.Embed)(handleError(err)));
...
async function 판매(interaction, bot) {
    try {
        ...
        await interaction.editReply((0, types_1.Embed)({
            ...
            icon: "white_check_mark",
            title: "판매 완료",
            description: `금 ${amount}개를 판매했습니다.\n판매 금액: \`${gold.sell.price.toLocaleString("ko-KR")} × ${amount.toLocaleString("ko-KR")} = ${(gold.sell.price * amount).toLocaleString("ko-KR")}원\`\n보유 중인 금: \`${userdata.gold.amount.toLocaleString("ko-KR")}개\`\n남은 돈: \`${userdata.money.amount.toLocaleString("ko-KR")}원\``,
        }));
    }
    catch (err) {
        await interaction.editReply((0, types_1.Embed)(handleError(err)));
...
function handleError(err) {
    ...
    if (err instanceof Error) {
        ...
        (0, types_1.errorLog)(err, "commands/gold");
        console.error(err);
    }
    return option;
}
