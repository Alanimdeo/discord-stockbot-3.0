...
async function 내주식(interaction, bot) {
    try {
        ...
        for (const code of codes) {
            ...
            const stockInfo = await (0, stock_1.getStockInfo)(code, bot.corpList);
            const currentPrice = stockInfo.price * userStock.amount;
            const avgPrice = userStock.buyPrice / userStock.amount;
            const plusMinus = currentPrice > userStock.buyPrice
                ? "+"
                : currentPrice === userStock.buyPrice
                    ? "="
                    : "";
            const prefix = stockInfo.price > avgPrice
                ? "diff\n-"
                : stockInfo.price === avgPrice
                    ? "\n*"
                    : "yaml\n=";
            const benefit = `${plusMinus}${Math.round(((stockInfo.price / avgPrice) * 100 - 100 + Number.EPSILON) * 100) / 100}%, ${plusMinus}${(currentPrice - userStock.buyPrice).toLocaleString("ko-KR")}원`;
            reply += `\`\`\`${prefix} ${stockInfo.name}(${stockInfo.code}): ${userStock.amount}주, ${currentPrice.toLocaleString("ko-KR")}원 (${currentPrice === userStock.buyPrice ? "=" : benefit})\n  구매 가격 ${userStock.buyPrice.toLocaleString("ko-KR")}원(평균 ${(Math.round((avgPrice + Number.EPSILON) * 100) / 100).toLocaleString("ko-KR")}원), 현재 1주당 ${stockInfo.price.toLocaleString("ko-KR")}원\`\`\``;
        }
...
async function 구매(interaction, bot) {
    try {
        ...
        await interaction.editReply((0, types_1.Embed)({
            ...
            title: "주식 구매 완료",
            description: `${stockInfo.name}(${stockInfo.code}) ${amount.toLocaleString("ko-KR")}주를 구매했습니다.\n구매 금액: \`${stockInfo.price.toLocaleString("ko-KR")} × ${amount.toLocaleString("ko-KR")} = ${(stockInfo.price * amount).toLocaleString("ko-KR")}원\`\n보유 중인 주식: \`${userdata.stock.status[stockInfo.code].amount.toLocaleString("ko-KR")}주\`\n남은 돈: \`${userdata.money.amount.toLocaleString("ko-KR")}원\``,
        }));
    }
    catch (err) {
        await interaction.editReply((0, types_1.Embed)(handleError(err)));
...
async function 판매(interaction, bot) {
    try {
        ...
        await interaction.editReply((0, types_1.Embed)({
            ...
            icon: "white_check_mark",
            title: "주식 판매 완료",
            description: `${stockInfo.name}(${stockInfo.code}) ${amount.toLocaleString("ko-KR")}주를 판매했습니다.\n판매 금액: \`${stockInfo.price.toLocaleString("ko-KR")} × ${amount.toLocaleString("ko-KR")} = ${(stockInfo.price * amount).toLocaleString("ko-KR")}원\`\n세금 및 수수료(0.3%): \`${fee.toLocaleString("ko-KR")}원\`\n실 수령액: \`${(stockInfo.price * amount - fee).toLocaleString("ko-KR")}원\`\n보유 중인 주식: \`${noStockRemain
                ? "0"
                : userdata.stock.status[stockInfo.code].amount.toLocaleString("ko-KR")}주\`\n남은 돈: \`${userdata.money.amount.toLocaleString("ko-KR")}원\``,
        }));
    }
    catch (err) {
        await interaction.editReply((0, types_1.Embed)(handleError(err)));
